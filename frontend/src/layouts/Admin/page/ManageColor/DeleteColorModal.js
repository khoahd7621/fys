...
import { deleteColorById } from '~/services/admin/colorService';

const DeleteColorModal = ({ show, setShow, data, setData, fetchList }) => {
  const handleClearData = () => {
    setData({});
  };

  const handleDeleteColor = async () => {
    const response = await deleteColorById(data.id);
        ...
        >
          <div className="relative p-4 w-full max-w-2xl h-auto">
            <div className="relative bg-white rounded-lg shadow">
              ...
              <div className="p-6 flex flex-col">Are you sure to delete color {data?.name}</div>
