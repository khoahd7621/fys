...
import { putUpdateColor } from '~/services/admin/colorService';

const UpdateColorModal = ({ show, setShow, data, setData, fetchList }) => {
  ...
  const handleUpdateColor = async () => {
    if (Validation.isEmpty(inputRef.current.value)) {
      ...
      const response = await putUpdateColor(data.id, String(inputRef.current.value).trim().toUpperCase());
          ...
          <div className="relative p-4 w-full max-w-2xl h-auto">
            <div className="relative bg-white rounded-lg shadow">
              <div className="flex justify-between items-start p-4 rounded-t border-b">
                <h3 className="text-xl font-semibold text-gray-900">Update color</h3>
                <button
