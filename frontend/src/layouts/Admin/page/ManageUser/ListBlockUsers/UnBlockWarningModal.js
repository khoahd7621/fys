...
import { putUnBlockUserByUserId } from '~/services/admin/userService';

const UnBlockWarningModal = ({ show, setShow, data, setData, fetchListUser }) => {
  const [isSending, setIsSending] = useState(false);

  const handleClickUnBlock = async () => {
    setIsSending(true);
    const response = await putUnBlockUserByUserId(data?.id);
      ...
      <div className="relative p-4 w-full max-w-2xl h-auto">
        <div className="relative bg-white rounded-lg shadow">
          ...
          <div className="px-4 py-2 flex flex-col">
            <div className="my-4">
              <div>Are you sure to un block user with email: {data.email}</div>
            </div>
          </div>
          <div className="flex items-center justify-center px-4 py-2 space-x-2 rounded-b border-t border-gray-200">
            ...
            <button
              className="w-1/2 my-2 mb-2 font-medium text-white bg-green-600 hover:bg-green-700 py-3"
...
export default UnBlockWarningModal;
