    ...
    <>
      ...
      <div className="text-white text-sm rounded-lg flex gap-3 w-full mb-4">
        <label htmlFor={`${title}-cover-image`} className="block rounded-sm cursor-pointer p-2 bg-green-600">
          {!image.previewImage && 'Add image'}
          {image.previewImage && 'Change image'}
        </label>
        {image.previewImage && (
      ...
      <div className="border-2 border-dashed rounded-lg p-2.5 h-64">
        {!image.previewImage && <span className="text-gray-400">No image choosen</span>}
        {image.previewImage && <img className="h-full mx-auto" src={image.previewImage} alt={title} title={title} />}
      </div>
    </>
  );
};
