...
import { deleteSizeById } from '~/services/admin/sizeService';

const DeleteSizeModal = ({ show, setShow, data, setData, fetchList }) => {
  const handleClearData = () => {
    setData({});
  };

  const handleDeleteSize = async () => {
    const response = await deleteSizeById(data.id);
        ...
        >
          <div className="relative p-4 w-full max-w-2xl h-auto">
            <div className="relative bg-white rounded-lg shadow">
              ...
              <div className="p-6 flex flex-col">Are you sure to delete size {data?.size}</div>
