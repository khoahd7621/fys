...
import { postCreateNewSize } from '~/services/admin/sizeService';

const AddNewSizeModal = ({ show, setShow, fetchList }) => {
  ...
  const handleCreateNewSize = async () => {
    if (Validation.isEmpty(inputRef.current.value)) {
      ...
      const response = await postCreateNewSize(String(inputRef.current.value).trim().toUpperCase());
          ...
          <div className="relative p-4 w-full max-w-2xl h-auto">
            <div className="relative bg-white rounded-lg shadow">
              <div className="flex justify-between items-start p-4 rounded-t border-b">
                <h3 className="text-xl font-semibold text-gray-900">Create new size</h3>
                <button
              ...
              <div className="p-6 flex flex-col">
                <div className="mb-6">
                  ...
                  <input
                    ...
                    placeholder="Enter clothes size"
                  />
                </div>
              </div>
